...
export default class Home extends Component{
  ...
  componentDidMount(){
    ...
    React.axios.all([
      ...
      React.axios({url:'/api/goods/home',params:{_page:1,_limit:15}})
    ]).then(React.axios.spread((banner, home)=>{//banner|home ~~ res
      banner.data.data.map(item => item.banner=this.baseUrl + '/' + item.banner );
      home.data.data.map(item => item.detail.auth_icon1=this.baseUrl + '/' + item.detail.auth_icon1 );
      home.data.data.map(item => item.detail.auth_icon2=this.baseUrl + '/' + item.detail.auth_icon2 );
      home.data.data.map(item => item.detail.auth_icon3=this.baseUrl + '/' + item.detail.auth_icon3 );
      this.setState({
